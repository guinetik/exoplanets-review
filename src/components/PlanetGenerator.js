...
class PlanetGenerator {
  ...
  generate_world(space, randomMass, planetData) {
    ...
    const north_pole = new Zdog.Shape({
      addTo: this.space,
      translate: { x: this.offset_x, y: this.offset_y },
      path: [{ y: this.planet_radius * -1 }, { y: (this.planet_radius + 50) * -1 }],
      stroke: 2,
      color: "firebrick",
    });
    ...
    const maxShrink = -3;
    const cols = [
      "rgba(255,255,255,0.1)", "rgba(255,255,255,0.2)", "rgba(255,255,255,0.3)",
      "rgba(160,130,52,1)", "rgba(160,130,52,1)", "rgba(0,128,0,1)",
      "rgba(85,107,47,1)", "rgba(67,85,37,1)", "rgba(160,100,42,1)",
      "rgba(140,80,42,1)", "rgba(120,60,42,1)", "rgba(170,110,92,1)",
      "rgba(200,180,200,1)"
    ];
  ...
  generateNeptuneLike() {
    const neptuneColors = [
      "rgba(255,255,255,0.2)", "rgba(200,200,200,0.1)", "#6FA5B1",
      "#5E8A93", "#6FA5B1", "#0c2325", "#1e5359", "rgba(0,0,0,0.1)"
    ];
    
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 110, sLng: index * 10, stages: 1 }, neptuneColors);
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 100, sLng: index * 10, stages: 1 }, neptuneColors);
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 120, sLng: index * 10, stages: 1 }, neptuneColors);
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 130, sLng: index * 10, stages: 1 }, neptuneColors);
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 140, sLng: index * 10, stages: 1 }, neptuneColors);
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 150, sLng: index * 10, stages: 1 }, neptuneColors);
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 180, sLng: index * 10, stages: 3 }, neptuneColors.reverse());
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: -100, sLng: index * 10, stages: 1 }, neptuneColors);
    }
  ...
  generateSuperEarth() {
    const superEarthColors = [
      "rgba(0,0,0,0.1)", "rgba(255,255,255,0.2)", "#6a351a",
      "#824121", "#6a351a", "#a32e2c", "#3c1010", "#361b0e",
      "rgba(160,100,42,1)", "rgba(140,80,42,1)", "rgba(120,60,42,1)",
      "rgba(170,110,92,1)", "rgba(200,200,200,0.5)"
    ];
    
    this.buildLandMass({ sLat: 20, sLng: 37, stages: 12 }, superEarthColors, false);
    this.buildLandMass({ sLat: 40, sLng: -37, stages: 12 }, superEarthColors, false);
    this.buildLandMass({ sLat: 30, sLng: -27, stages: 12 }, superEarthColors, false);
    this.buildLandMass({ sLat: 0, sLng: 150, stages: 8 }, superEarthColors, false);
    this.buildLandMass({ sLat: 60, sLng: -70, stages: 12 }, superEarthColors, false);
    this.buildLandMass({ sLat: 10, sLng: -10, stages: 8 }, superEarthColors, false);
    this.buildLandMass({ sLat: -50, sLng: -130, stages: 9 }, superEarthColors, true);
    this.buildLandMass({ sLat: 20, sLng: -90, stages: 6 }, superEarthColors, true);
    this.buildLandMass({ sLat: -10, sLng: 10, stages: 12 }, superEarthColors, false);
    this.buildLandMass({ sLat: -15, sLng: 180, stages: 12 }, superEarthColors, false);
    this.buildLandMass({ sLat: 35, sLng: 80, stages: 9 }, superEarthColors, true);
  }

  generateGasGiant() {
    const gasGiantColors = [
      "rgba(0,0,0,0.1)", "#dbc8cf", "#e1bda6", "#be947b",
      "#a66d65", "#e87849", "#ab4219", "#be5c2a", "#a54426", "#a54426"
    ];
    const darkColors = [
      "#402323", "#613734", "rgba(255,255,255,0.5)", "#dbc8cf"
    ];
    
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 180, sLng: index * 10, stages: 4 }, gasGiantColors);
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 140, sLng: index * 10, stages: 1 }, darkColors);
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 130, sLng: index * 10, stages: 1 }, darkColors);
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 120, sLng: index * 10, stages: 1 }, darkColors);
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 110, sLng: index * 10, stages: 1 }, darkColors.reverse());
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: 100, sLng: index * 10, stages: 1 }, darkColors.reverse());
    }
    for (let index = 0; index < 36; index++) {
      this.buildLandMass({ sLat: -100, sLng: index * 10, stages: 1 }, darkColors.reverse());
    }
